...
export class Commitizen {
  ...
  public async getType(): Promise<void> {
      await askOneOf('type用于说明commit的类型', this.configs.types,
  ...
  public async getScope(): Promise<void> {
    ...
    await askOneOf('scope用于说明本次Commit所影响的范围', this.configs.scopes,
      pick => {
        this.scope = pick.label;
      },
      '自定义scope', '输入一个新的scope');
  }
  public async getSubject(): Promise<void> {
      ...
      await ask('subject是本次commit目的的简短描述，一般不要超过50个字符',
